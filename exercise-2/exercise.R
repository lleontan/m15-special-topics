...
for(i in 1:100) {
  ...
  data_ind<-sample(seq_len(nrow(homes)),size=sample.size)
  testing<-homes[-data_ind,]
  training<-homes[data_ind,]
  ...
  assessment<-AssessFit(basic.fit,data=testing)
  
  # 4. Store your assessment in the `basic.fits` vector
  basic.fit<-c(basic.fits,assessment)
}
